...
- Q: When Active primitives are under-linked (missing canonical required relationships), how should validation.json and integrity-report.json split responsibility? → A: Validation owns Active under-linked failures in `validation.json` only; `integrity-report.json` contains integrity-specific findings only (orphans, duplicate names, derivability gaps, etc.) without duplicating Active validation failure entries. Gap sections in `summary.md` may reference both sources.
- Q: For Active Workflows missing explicit expectedResults or domainConcepts relations (FR-016), when should integrity treat them as derivable vs missing? → A: Derivable when traversing the workflow's linked Capabilities yields ≥1 Expected Result and ≥1 Domain Concept (via capability refs or Capability Concept Links); otherwise emit integrity warning `missing-workflow-derivation`.
- Q: For integrity reporting, what defines an orphan primitive? → A: Report as orphan only when a primitive has zero inbound and zero outbound typed relationship edges **and** its type cannot meaningfully stand alone without relationships. Actors MAY remain disconnected during drafting without orphan classification; Draft Objectives MAY stand alone (FR-021). Orphans are integrity **warnings**, not validation failures.
- Q: For likely-duplicate detection (fuzzy name pairs within the same primitive type), which similarity rule should v0 use? → A: Jaccard similarity ≥ 0.8 on word tokens (split on whitespace after trim+lowercase normalization); reported as integrity warnings only.
- Q: For duplicate-name and likely-duplicate detection, how should primitive display names be normalized before comparison? → A: Trim leading/trailing whitespace and lowercase the full string (e.g., `" Schedule Session "` → `"schedule session"`); internal spacing and punctuation are preserved.
...
2. **Given** Active primitives with no inbound or outbound relationships where canonical rules require them, **When** the user requests the report, **Then** under-linked Active primitives are reported as validation failures in `validation.json` only; `integrity-report.json` does not duplicate them; under-linked Draft primitives appear as validation warnings.
...
- **Duplicate normalized names**: Same normalized display name within a primitive type (e.g., two Active capabilities named "Schedule Session") are integrity **warnings**, not validation or integrity failures; they appear in the integrity report and gap sections but do not alone fail `check` or force exit code `1`. Name normalization for comparison is **trim + lowercase** on the full display name; internal spacing and punctuation are preserved (e.g., `"Schedule  Session"` and `"Schedule Session"` remain distinct).
...
- **Orphaned primitives**: Integrity reports a primitive as **orphan** only when it has zero inbound and zero outbound typed relationship edges **and** its type cannot meaningfully stand alone without relationships. Disconnected **Actors** are not orphans—Actors may be identified during planning before stories and workflows are modeled. **Draft Objectives** may stand alone without orphan classification (FR-021). Join-like types (e.g., Capability Concept Link, Story) and relationship-dependent types without edges are orphans. Orphans are integrity **warnings**; Active missing required relationships remain validation failures (FR-007), not orphans.
...
- **FR-009**: Validation output MUST distinguish warnings (Draft incompleteness, per-primitive advisory quality flags per FR-013–FR-026) from failures (Active incompleteness, broken references, duplicate IDs, duplicate Active story triples). Per-primitive advisories appear in `validation.json`; cross-primitive graph heuristics (e.g., duplicate names, orphans, workflow derivability per FR-016) appear in `integrity-report.json`. Active under-linked failures MUST NOT be duplicated in `integrity-report.json`.
...
- **FR-016**: Expected Results and Domain Concepts MAY be explicit workflow relations or derived from workflow capabilities; integrity reporting MUST warn when they are missing or not derivable (cross-primitive heuristic reported in `integrity-report.json`). A workflow's Expected Results and Domain Concepts are **derivable** when traversing its linked Capabilities yields ≥1 Expected Result (via capability `expectedResults` refs) and ≥1 Domain Concept (via capability `domainConcepts` refs or Capability Concept Links); otherwise emit integrity warning `missing-workflow-derivation`.
...
- **FR-034**: The integrity report MUST consolidate integrity-specific graph heuristics: orphans, broken references not already in validation, duplicate names within a primitive type, likely duplicates, and cross-primitive advisory quality warnings (e.g., workflow derivation gaps per FR-016) with status-aware severity. Active under-linked failures are owned by validation (`validation.json` per FR-007/FR-009) and MUST NOT be duplicated as integrity failure entries. Duplicate Active story triple **failures** are owned by validation (`validation.json` per FR-011); the integrity report includes a `duplicateStoryTriples` summary section for fix-up context only. Likely-duplicate detection MUST use Jaccard similarity ≥ 0.8 on word tokens (whitespace-split after trim+lowercase normalization) for same-type name pairs; findings are integrity **warnings** only. **Orphan** findings apply only to primitives with zero inbound and zero outbound typed relationship edges whose type cannot meaningfully stand alone without relationships; disconnected Actors and standalone Draft Objectives MUST NOT be classified as orphans.
- **FR-034a**: Duplicate normalized names within a primitive type MUST be reported as integrity **warnings**, not validation failures or integrity failures. Duplicate names alone MUST NOT cause a non-zero CLI exit code. Normalization for name comparison MUST trim leading/trailing whitespace and lowercase the full display name; internal spacing and punctuation are preserved.
...
- **FR-039**: The summary MUST surface modeling gaps from validation and integrity findings rather than omitting or fabricating missing sections; Active failures and Draft warnings MUST be visually distinguishable in gap reporting. Gap sections MAY reference both `validation.json` failures/warnings and `integrity-report.json` integrity-specific findings without duplicating full finding objects.
...
- Exact duplicate normalized names within a primitive type are integrity **warnings** (not failures). Name normalization is trim + lowercase on the full display name; internal spacing and punctuation are preserved. Likely-duplicate detection uses Jaccard similarity ≥ 0.8 on word tokens (whitespace-split after trim+lowercase) for same-type pairs; findings are integrity **warnings** only.
- Disconnected primitives are expected during drafting. Integrity orphan findings apply only when a primitive has zero relationship edges and its type cannot meaningfully stand alone (e.g., Story, Capability Concept Link). Actors may exist without links while planning; Draft Objectives may stand alone.
