...
**Tests**: Per SpecAble constitution, tests for schema validation, graph traversal, missing-link detection, duplicate-name detection, artifact generation, and adapter behavior are REQUIRED unless the feature spec explicitly excludes a category with rationale.
...
- **SpecAble default (library-first monorepo)**: `packages/<name>/src/`, `packages/<name>/tests/`, thin adapters in `apps/cli/` or `apps/mcp/`
- **Single project**: `src/`, `tests/` at repository root (only if plan.md explicitly chooses this)
